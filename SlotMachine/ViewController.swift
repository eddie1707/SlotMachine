...
//  SlotMachine
//
//  Created by Edwin Wiersma on 02/10/14.
...
class ViewController: UIViewController {
    ...
    func betOneButtonPressed(button: UIButton) {
        if credtis <= 0 {
            showAlertWithText(header: "No more credits", message: "You need to reset the game.")
        } else {
            if currentBet < 5 {
                currentBet += 1
                credtis -= 1
    ...
    func updateMainView() {
        self.creditsLabel.text = "\(credtis)"
